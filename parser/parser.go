...
func (p *parser) parseLibBody(f *ast.File, prefix string) {
	...
	for p.peek().Kind != RBrace && p.peek().Kind != EOF {
		...
		if p.peek().Kind == Ident && p.peek().Value == "rwir" {
			...
			f.RwirDecls = append(f.RwirDecls, decl)
			p.skipNewlines()
			continue // 缺 continue 会落到下面的 parseStmt，往 InitBody 塞一条幻影
			         // 空指令，使 /lib/init/[0,0] 变成空 opcode → vthread 立刻
			         // SetDone("ok")，整个程序静默变成空操作
		} else if p.peek().Kind == Ident && p.peek().Value == "rwfunc" {
			fn := p.parseFunc()
...
func (p *parser) parseDottedTail() string {
	...
	for p.peek().Kind == Dot {
		...
		if p.peek().Kind != Ident {
			p.errors = append(p.errors, Diagnostic{Pos: dot.Pos,
				Message: "SyntaxError: 名字里的 '.' 后面缺少标识符；help: 写成 pkg.name"})
			return tail
		}
		tail += dot.Value + p.advance().Value
	}
	return tail
}
...
func (p *parser) parseFuncSig() ast.FuncSig {
	// 关键字既可能是 rwfunc，也可能是 KV 里存的 rwir 签名串开头的 rwir
	// （ParseFuncSig 按存下来的字符串重新解析）。注意目前没有任何运行时路径
	// 会把 rwir 签名串喂进来：HandleCall 被 IsDelegated 短路，Bootstrap 只在
	// 有实参时才解析而 CLI 传的是 nil。这里支持点号名字是为了 rwfunc 侧对齐，
	// 不要据此以为它在为委托服务。
	kw := p.advance()
	var sig ast.FuncSig
	if t := p.peek(); t.Kind == Ident {
