...
// writeSlotChecked 解析写槽并校验落点（走 ‥wparam 零拷贝重定向那一条）。
//
// 注意本仓库有**两条**写解析路径：这一条和 resolve.go 的 writeSlotKey。
// 两条都必须校验 —— 只守一条等于没守，换个算子就绕过去了
// （`{k=v} -> /lib/pwned`、`set("/lib/mylib","add",v)` 都不走这一条）。
